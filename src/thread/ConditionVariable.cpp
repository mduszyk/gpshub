...
ConditionVariable::ConditionVariable() {
    ...
    if ((i = pthread_cond_init(&cond, NULL)) != 0) {
        throw ThreadSyncException(
            "Failed initializing condition variable, return value: " + i);
    }
}
...
ConditionVariable::~ConditionVariable() {
    ...
    if ((i = pthread_cond_destroy(&cond)) != 0) {
        throw ThreadSyncException(
            "Failed destroying condition variable, return value: " + i);
    }
}
...
void ConditionVariable::signal() {
    ...
    if ((i = pthread_cond_signal(&cond)) != 0) {
        throw ThreadSyncException(
            "Failed signalling condition variable, return value: " + i);
    }
}
...
void ConditionVariable::wait(Mutex& mtx) {
    ...
    if ((i = pthread_cond_wait(&cond, mtx.getPthreadPtr())) != 0) {
        throw ThreadSyncException(
            "Failed waiting on condition variable, return value: " + i);
    }
}
